...
def validation(model_name, expert_fns, config, seed=""):
    ...
    with open(
        config["ckp_dir"] + "true_label_multiple_experts_new" + model_name + ".txt", "w"
    ) as f:
        json.dump(json.dumps(true_label, cls=NumpyEncoder), f)

    with open(
        config["ckp_dir"] + "confidence_multiple_experts_new" + model_name + ".txt", "w"
    ) as f:
        json.dump(json.dumps(classifier_confidence, cls=NumpyEncoder), f)

    with open(
        config["ckp_dir"]
        + "expert_predictions_multiple_experts_new"
        + model_name
        + ".txt",
        "w",
    ) as f:
    ...
    with open(
        config["ckp_dir"] + "validation_multiple_experts_new" + model_name + ".txt", "w"
    ) as f:
