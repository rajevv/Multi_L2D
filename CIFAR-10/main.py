...
def train(model,
	...
	for epoch in range(0, config["epochs"]):
		...
		if validation_loss < best_validation_loss:
			...
			torch.save(model.state_dict(), os.path.join(config["ckp_dir"], config["experiment_name"] + '_' + str(len(expert_fns)) + '_experts' + '.pt'))
			# Additionally save the whole config dict
			with open(os.path.join(config["ckp_dir"], config["experiment_name"] + '_' + str(len(expert_fns)) + '_experts' + '.json'), "w") as f:
				json.dump(config, f)
			patience = 0
		else:
			patience += 1
