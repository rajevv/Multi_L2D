...
def hatespeech():
    ...
    def increase_sys_acc():
        ...
        # bax = brokenaxes(ylims=((30, 50), (70, 94)), hspace=.05)
        f_acc, ax_acc = plt.subplots(1, 1, figsize=(2.5, 2.5))
        ...
        # ====================== #
        # # OvA
        # bax.errorbar(exp_list, ova_sys_acc_mean, yerr=ova_sys_acc_std,
        #              alpha=0.75, label="OvA", **ova_args)
        #
        # # Softmax
        # bax.errorbar(exp_list, softmax_sys_acc_mean, yerr=softmax_sys_acc_std,
        #              alpha=0.75, label="Softmax", **softmax_args)
        #
        # # Hemmer
        # bax.errorbar(exp_list, hemmer_sys_acc_mean, yerr=hemmer_sys_acc_std,
        #              alpha=0.75, label="Softmax", **hemmer_args)
        #
        # # One Classifier
        # bax.errorbar(exp_list, oneclf_sys_ac_mean, yerr=oneclf_sys_acc_std,
        #              alpha=0.75, label="Softmax", **oneclf_args)
        #
        # # Best Expert
        # bax.errorbar(exp_list, best_expert_sys_acc_mean, yerr=best_expert_sys_acc_std,
        #              alpha=0.75, label="Softmax", **best_expert_args)
        #
        # bax.set_xticks(exp_list, exp_list)
        # bax.set_yticks(list(plt.yticks()[0])[::2])
        # # ax.set_ylim((ax.get_ylim()[0], 100))
        # bax.set_ylabel(r"System Accuracy $(\%)$")
        # bax.set_xlabel(r"Number of Experts")
        # bax.grid()
        ...
        # OvA
        ax_ece.errorbar(exp_list, ova_ece_mean, yerr=ova_ece_std,
                        alpha=0.75, label="OvA", **ova_args)

        # Softmax
        ax_ece.errorbar(exp_list, softmax_ece_mean, yerr=softmax_ece_std,
                        alpha=0.75, label="Softmax", **softmax_args)

        # Hemmer
        ax_ece.errorbar(exp_list, hemmer_ece_mean, yerr=hemmer_ece_std,
                        alpha=0.75, label="Softmax", **hemmer_args)
        ax_ece.set_xticks(exp_list, exp_list)
        ax_ece.set_yticks(plt.yticks()[0][1:])
        # ax.set_ylim((ax.get_ylim()[0], 100))
        ax_ece.set_ylabel(r"Average ECE $(\%)$")
        ax_ece.set_xlabel(r"Number of Experts")
        ax_ece.grid()
        ova_leg = mlines.Line2D([], [], linestyle='-', label='OvA', **ova_args)
        softmax_leg = mlines.Line2D([], [], linestyle='-', label='Softmax', **softmax_args)
        hemmer_leg = mlines.Line2D([], [], linestyle='-', label='Hemmer', **hemmer_args)
        legend_list = [ova_leg, softmax_leg, hemmer_leg]
        ax_ece.legend(handles=legend_list, loc="best")
        return f_ece, ax_ece

    f, ax = increase_sys_acc()
    f.set_tight_layout(True)
    plt.show()
    f.savefig(paper_results_path + "hatespeech_ece_experts.pdf")
