'''
script to run the CIFAR-10 experiment where the number of oracle experts are increased from k=1 to k=10.
...
def increaseOracles(config):
    ...
    for seed in [948, 625, 436, 791, 1750]:  # ,  812, 1331, 1617,  650, 1816]:
        ...
        for k in range(config["n_classes"]):
            ...
            # list of experts
            expert_fns = [None] * num_experts
            idx = np.array(range(num_experts))
            # Oracles ===
            ...
            expert_fn_notOracle = getattr(expert_notOracle, 'predict_prob_cifar_2')

            # randomly select positions of the oracle experts
            idx_oracles = np.array(random.sample(range(num_experts), k))
            idx_random = list(set(idx) - set(idx_oracles))

            # add experts
            # random
            for i_random in idx_random:
                expert_fns[i_random] = expert_fn_notOracle
            # oracles
            for i_oracle in idx_oracles:
                expert_fns[i_oracle] = expert_fn_oracle

            log['oracles_positions'][k] = idx_oracles.tolist()
