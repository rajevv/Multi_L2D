'''
script to run the CIFAR-10 experiment where experts go for specialized in one non-overlapping class to 
...
# === Experiment 3 === # (see the description at the top.)
def gradualOverlapping(config):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--expert_type", type=str, default="predict_prob_cifar",
                        help="specify the expert type. For the type of experts available, see-> models -> experts. defualt=predict_prob_cifar.")
    ...
    parser.add_argument("--experiment_name", type=str, default="gradual_overlap",
                        help="specify the experiment name. Checkpoints will be saved with this name.")

    config = parser.parse_args().__dict__

    print(config)
    gradualOverlapping(config)
