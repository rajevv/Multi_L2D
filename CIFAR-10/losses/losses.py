...
class Criterion(object):
    ...
    def ova(self, outputs, labels, collection_Ms, n_classes):
        ...
        l1 = self.LogisticLoss(outputs[range(batch_size), labels], 1)
        l2 = torch.sum(self.LogisticLoss(outputs[:, :(n_classes - len(collection_Ms))], -1), dim=1) - self.LogisticLoss(
            outputs[range(batch_size), labels], -1)

        l3 = 0
        for j in range(num_experts):
            l3 += self.LogisticLoss(outputs[range(batch_size), n_classes - 1 - j], -1)

        l4 = 0
        for j in range(num_experts):
            l4 += collection_Ms[j][0] * (self.LogisticLoss(outputs[range(batch_size), n_classes - 1 - j], 1)
                                         - self.LogisticLoss(outputs[range(batch_size), n_classes - 1 - j], -1))
