...
def increase_experts(config):
	config["ckp_dir"] = "./" + config["loss_type"] + "_increase_experts_nonoverlapping_one2two"
	...
	for i,n in enumerate(experiment_experts,0):
		...
		num_experts = n
		expert = synth_expert2(S=np.arange(i*2, i*2 + 2), p_in = 1.0, p_out = 0.0, n_classes=config["n_classes"])
		expert_fn = getattr(expert, config["expert_type"])
		expert_fns.append(expert_fn)
