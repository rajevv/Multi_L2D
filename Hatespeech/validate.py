...
def main_validate_best_expert(testD, expert_fns, config):
	...
	test_dl = torch.utils.data.DataLoader(testD, batch_size=config["batch_size"], shuffle=False, drop_last=True, **kwargs)
...
def validate_best_expert(config):
	testD = HatespeechDataset(split='test')
...
def forward_surrogate(model, dataloader, expert_fns, config):
	...
	confidence = torch.stack(confidence, dim=0).view(-1, config["n_classes"] + len(expert_fns))
...
def main_validate_surrogate(model, testD, expert_fns, config, seed=''):
	def get(severity, dl):
		true, confidence, expert_predictions = forward_surrogate(model, dl, expert_fns, config)
		...
		print("Evaluate...")
		result_ = main_increase_experts.evaluate(model, expert_fns, loss_fn, config["n_classes"]+len(expert_fns), dl, config)
		#print(result_)
	...
	model_name = config["experiment_name"] + '_' + str(len(expert_fns)) + '_experts' + '_seed_' + str(seed)
...
def validate_surrogate(config):
	...
	for seed in ['', 948,  625,  436,  791]:
		...
		for i,n in enumerate(experiment_experts):
			...
			model = CNN_rej(embedding_dim=100, vocab_size=100, n_filters=300, filter_sizes=[3,4,5], dropout=0.5, output_dim=int(config["n_classes"]), num_experts=num_experts) 
			testD = HatespeechDataset(split='test')
			result = main_validate_surrogate(model, testD, expert_fns, config, seed=seed)
...
def main_validate_hemmer(model, testD, expert_fns, config, seed=''):
	def get(severity, dl):
		...
		loss_fn = nn.NLLLoss()
		print("Evaluate...")
		result_ = hemmer_baseline.evaluate(model, expert_fns, loss_fn, config["n_classes"]+len(expert_fns), dl, config)
		#print(result_)
		result[severity]  = filter(result_)
		true_label[severity] = true.numpy()
...
def main_validate_classifier(model, testD, expert_fns, config, seed=''):
	...
	result = {}
	result_ = main_classifier.evaluate(model, expert_fns, nn.NLLLoss(), config["n_classes"], test_dl, config)
...
def validate_classifier(config):
	...
	expert_fns = []
	accuracy = []
	for seed in ['', 948, 625, 436, 791]: #, 948,  625,  436,  791]: #, 1750,  812, 1331, 1617,  650, 1816]:
		print("run for seed {}".format(seed))
		if seed != '':
			set_seed(seed)
		model = CNN_clf(embedding_dim=100, vocab_size=100, n_filters=300, filter_sizes=[3,4,5], dropout=0.5, output_dim=int(config["n_classes"])) 
		testD = HatespeechDataset(split='test')
