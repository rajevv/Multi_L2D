...
class MLPMixer(nn.Module):
    def __init__(
        ...
        self.mixers = nn.Sequential(
            *[
                MixerLayer(num_patches, num_features,
...
def train(config, device, model, trainloader, validloader, ckp_path):
    ...
    optimizer = torch.optim.Adam(
        model.parameters(), lr=config['lrate'], weight_decay=config['weight_decay'])
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, num_batches * config['epochs'])
    metrics = Metrics()
    train_losses = []
    val_losses = []
    best_val_loss = 1000
    ...
    for epoch in range(config['epochs']):
        ...
        for i, batch in enumerate(trainloader):
            ...
            iter += 1
        print('[{}]/[{}] Train loss: {}'.format(epoch,
              config['epochs'], epoch_loss / num_batches),  flush=True)
        ...
        with torch.no_grad():
            ...
            print('Validation loss: {}, Validation accuracy: {}'.format(
                float(val_loss), float(val_acc)), flush=True)
...
if __name__ == "__main__":
    ...
    model = MLPMixer(image_size=224, patch_size=16, in_channels=3,
                     num_features=128, expansion_factor=2, num_layers=8, num_classes=7)
    kwargs = {'num_workers': 0, 'pin_memory': True}
    trainloader = torch.utils.data.DataLoader(train_data,
                                              batch_size=batch_size, shuffle=True, drop_last=False, **kwargs)
    validloader = torch.utils.data.DataLoader(
        val_data, batch_size=batch_size, shuffle=True, drop_last=False, **kwargs)
    run = wandb.init(project="ham10000 expert", reinit=True,
                     config=config, entity="aritzz")
