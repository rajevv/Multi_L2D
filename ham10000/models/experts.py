...
class synth_expert_hard_coded:
    ...
    def __init__(self, p_in=0.7, p_out=1/7, k=[3], device="cpu"):
        ...
        self.expert = MLPMixer(image_size=224, patch_size=16, in_channels=3,
                               num_features=128, expansion_factor=2, num_layers=8, num_classes=7).to(device)
        self.expert.load_state_dict(torch.load(
            './MLP_Mixer_model/m_expert', map_location=device))
        self.expert.eval()
        self.n_classes = 7
    ...
    def MLPMixer(self, X, labels, Z):
        ...
        # conf.multinomial(num_samples=1, replacement=True)
        c, outs = torch.max(conf, dim=1)
            ...
            if labels[i].item() in self.S:
                ...
                if coin_flip == 0:
                    ...
                    if len(in_list) == 0:  # empty
                        in_list = list(
                            set(range(self.n_classes)) - set([labels[i].item()]))
                ...
                if coin_flip == 0:
                    ...
                    out_list = list(set(range(self.n_classes)) -
                                    set(self.S + [labels[i].item()]))
