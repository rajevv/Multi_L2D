...
class cifar(Dataset):
    ...
    @classmethod
    def read(cls, severity=0, slice_=-1, test=False, data_aug=False, entropy=False, mix=False, only_id=False,
             only_ood=False, split_train_set=False, train_proportion=1.0):
        ...
        if not test:
            if only_ood:
                ...
                return cls(ood_train_data, ood_train_labels, data_aug=data_aug), cls(ood_val_data, ood_val_labels,
                                                                                     data_aug=False)

            if only_id:
                (id_train_data, id_train_labels), (id_val_data, id_val_labels) = cifar.ID(test)
                return cls(id_train_data, id_train_labels, data_aug=data_aug, split_train_set=split_train_set,
                           train_proportion=train_proportion), cls(id_val_data, id_val_labels, data_aug=False)

            if entropy:
                (ood_train_data, ood_train_labels), (ood_val_data, ood_val_labels) = cifar.OOD(severity, slice_, test)
                (id_train_data, id_train_labels), (id_val_data, id_val_labels) = cifar.ID(test)
                return cls(id_val_data, id_val_labels, data_aug=False), cls(ood_val_data, ood_val_labels,
                                                                            data_aug=False)
