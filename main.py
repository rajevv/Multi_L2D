...
def train_reject(iters, warmup_iters, lrate, train_loader, model, optimizer, scheduler, epoch, num_experts, expert_fns, n_classes, alpha):
...
def run_reject(model, train_dataset, valid_dataset, n_classes, num_experts, expert_fns, epochs, alpha, batch_size, k, save_path='./'):
	...
	for epoch in range(0, epochs):
		# train for one epoch
		iters, train_loss = train_reject(iters, warmup_iters, lrate, train_loader, model, optimizer, scheduler, epoch, num_experts, expert_fns, n_classes, alpha)
...
if __name__ == "__main__":
	...
	for exp in tqdm(range(0,trials)):
		...
		cluster3_labels_valid = torch.ones([math.floor(total_samples * (1-group_proportion) * 0.5 )], dtype=torch.long) + 1.0
		...
		cluster4_labels_valid = torch.ones([math.floor(total_samples * (1-group_proportion) * 0.5 )], dtype=torch.long) + 2.0
		data_x_valid = torch.cat([cluster1_valid, cluster2_valid, cluster3_valid, cluster4_valid])
		data_y_valid = torch.cat([cluster1_labels_valid, cluster2_labels_valid, cluster3_labels_valid, cluster4_labels_valid])
		...
		cluster3_labels_test = torch.ones([math.floor(total_samples * (1-group_proportion) * 0.5 )], dtype=torch.long) + 1.0
		...
		cluster4_labels_test = torch.ones([math.floor(total_samples * (1-group_proportion) * 0.5 )], dtype=torch.long) + 2.0
	...
	for n in [2,4,6,8]:
		...
		run_reject(model,train_d, valid_d, n_dataset+num_experts, num_experts, expert_fns, 200, alpha, bsz, k, save_path=path) # train for 200 epochs